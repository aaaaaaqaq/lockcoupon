...
export default function CouponCard({ coupon, onOpenPopup }: CouponCardProps) {
  ...
  const partialCode = coupon.code ? coupon.code.slice(-2) : '';

  return (
    <div className="bg-white border border-border rounded-xl overflow-hidden hover:shadow-md transition-all group">
      <div className="flex items-stretch">
        ...
        {/* Right — CTA button */}
        <div className="w-[110px] sm:w-[140px] shrink-0 flex flex-col items-center justify-center p-3 sm:p-4">
          <button
            onClick={() => onOpenPopup(coupon)}
            className="w-full bg-primary hover:bg-primary-dark text-white font-bold text-[12px] sm:text-[13px] py-2.5 sm:py-3 rounded-lg transition-all flex items-center justify-center gap-0 overflow-hidden"
          >
            <span className="flex-1 text-center">Voir le code &gt;</span>
            {partialCode && (
              <span className="bg-primary-dark text-white/80 text-[11px] sm:text-[12px] font-mono py-2.5 sm:py-3 px-2 -my-2.5 sm:-my-3 -mr-0 ml-1 border-l border-white/20">
                {partialCode}
              </span>
            )}
          </button>
